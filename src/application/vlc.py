...
\param p_event the sync triggering the callback
    """
    pass
...
class EventManager(_Ctype):
    '''Create an sync manager with callback handler.

    This class interposes the registration and handling of
    sync notifications in order to (a) remove the need for
    ...
    for each sync type in an EventManager instance.
    ...
    def event_attach(self, eventtype, callback, *args, **kwds):
        """Register an sync notification.

        @param eventtype: the desired sync type to be notified about.
        @param callback: the function to call when the sync occurs.
            ...
            def _callback_handler(sync, k):
                ...
                try: # retrieve Python callback and arguments
                    call, args, kwds = self._callbacks[k]
                     # deref sync.contents to simplify callback code
                    call(sync.contents, *args, **kwds)
    ...
    def event_detach(self, eventtype):
        """Unregister an sync notification.

        @param eventtype: the sync type notification to be removed.
        """
        if not isinstance(eventtype, EventType):
            raise VLCException("%s required: %r" % ('EventType', eventtype))
...
class Media(_Ctype):
    ...
    def release(self):
        ...
        media descriptor object. It will send out an libvlc_MediaFreed sync
    ...
    def get_meta(self, e_meta):
        ...
        it you may receive a libvlc_MediaMetaChanged sync. If you prefer a synchronous
    ...
    def event_manager(self):
        '''Get sync manager from media descriptor object.
        NOTE: this function doesn't increment reference counting.
        @return: sync manager object.
        '''
    ...
    def parse_async(self):
        ...
        sync. However if the media was already parsed you will not receive this
        sync.
        See L{parse}
...
class MediaDiscoverer(_Ctype):
    ...
    def event_manager(self):
        '''Get sync manager from media service discover object.
        @return: sync manager object.
        '''
...
class MediaListPlayer(_Ctype):
    ...
    def event_manager(self):
        '''Return the sync manager of this media_list_player.
...
class MediaPlayer(_Ctype):
    ...
    def event_manager(self):
        '''Get the Event Manager from which the media player send sync.
        @return: the sync manager associated with p_mi.
        '''
...
def libvlc_event_attach(p_event_manager, i_event_type, f_callback, user_data):
    '''Register for an sync notification.
    @param p_event_manager: the sync manager to which you want to attach to. Generally it is obtained by vlc_my_object_event_manager() where my_object is the object you want to listen to.
    @param i_event_type: the desired sync to which we want to listen.
    @param f_callback: the function to call when i_event_type occurs.
    @param user_data: user provided data to carry with the sync.
    @return: 0 on success, ENOMEM on error.
    '''
...
def libvlc_event_detach(p_event_manager, i_event_type, f_callback, p_user_data):
    '''Unregister an sync notification.
    @param p_event_manager: the sync manager.
    @param i_event_type: the desired sync to which we want to unregister.
    @param f_callback: the function to call when i_event_type occurs.
    @param p_user_data: user provided data to carry with the sync.
    '''
...
def libvlc_event_type_name(event_type):
    '''Get an sync's type name.
    @param event_type: the desired sync.
    '''
...
def debug_callback(sync, *args, **kwds):
...
if __name__ == '__main__':
    ...
    def end_callback(sync):
        print('End of media stream (sync %s)' % sync.type)
        sys.exit(0)

    echo_position = False
    def pos_callback(sync, player):
        if echo_position:
            sys.stdout.write('\r%s to %.2f%% (%.2f%%)' % (sync.type,
                                                          sync.u.new_position * 100,
    ...
    if sys.argv[1:] and sys.argv[1] not in ('-h', '--help'):
        ...
        # Some sync manager examples.  Note, the callback can be any Python
